...
const src = PNG.sync.read(fs.readFileSync(path.join(tilesDir, 'tile-single.png')));

// 1. Clean any stray alpha noise around borders of tile-single
const cleanedTile = new PNG({ width: src.width, height: src.height });
for (let y = 0; y < src.height; y++) {
  for (let x = 0; x < src.width; x++) {
    ...
    // Remove any isolated stray line noise on edges
    if (x <= 1 || x >= src.width - 2 || y <= 1 || y >= src.height - 2) {
      cleanedTile.data[idx + 3] = 0;
    } else {
      cleanedTile.data[idx] = r;
      cleanedTile.data[idx + 1] = g;
      cleanedTile.data[idx + 2] = b;
      cleanedTile.data[idx + 3] = a;
    }
  }
}

// 2. Measure geometry of single cube top diamond
let topPt = { x: 0, y: cleanedTile.height };
let leftPt = { x: cleanedTile.width, y: 0 };
let rightPt = { x: 0, y: 0 };

for (let y = 0; y < cleanedTile.height; y++) {
  for (let x = 0; x < cleanedTile.width; x++) {
    const a = cleanedTile.data[(cleanedTile.width * y + x) * 4 + 3];
    if (a > 150) {
      if (y < topPt.y) topPt = { x, y };
      if (x < leftPt.x) leftPt = { x, y };
      if (x > rightPt.x) rightPt = { x, y };
    }
  }
}
...
// Calculate step vector: from topPt to rightPt and topPt to leftPt
const stepX = { dx: (rightPt.x - topPt.x) * 0.46, dy: (rightPt.y - topPt.y) * 0.46 };
const stepY = { dx: (leftPt.x - topPt.x) * 0.46, dy: (leftPt.y - topPt.y) * 0.46 };

console.log('Step X (SE vector):', stepX);
console.log('Step Y (SW vector):', stepY);

// Scale tile for assembly
...
const SCALE = 0.52;
const tileScaled = resizePng(cleanedTile, SCALE);
  ...
  for (let sy = 0; sy < srcImg.height; sy++) {
    for (let sx = 0; sx < srcImg.width; sx++) {
      ...
      if (da === 0) {
        ...
        const alpha = sa / 255;
        const inv = 1 - alpha;
        dstImg.data[dIdx] = Math.round(srcImg.data[sIdx] * alpha + dstImg.data[dIdx] * inv);
        dstImg.data[dIdx + 1] = Math.round(srcImg.data[sIdx + 1] * alpha + dstImg.data[dIdx + 1] * inv);
        dstImg.data[dIdx + 2] = Math.round(srcImg.data[sIdx + 2] * alpha + dstImg.data[dIdx + 2] * inv);
...
// 4x4 Grid = 16 Cubes, sorted back-to-front by (x + y)
const GRID_SIZE = 4;
const positions = [];
...
const ORIGIN_X = 400 - tileScaled.width / 2;
const ORIGIN_Y = 120;

// Exact step based on top diamond facet
const DX_SE = (rightPt.x - topPt.x) * SCALE * 0.985;
const DY_SE = (rightPt.y - topPt.y) * SCALE * 0.985;
const DX_SW = (leftPt.x - topPt.x) * SCALE * 0.985;
const DY_SW = (leftPt.y - topPt.y) * SCALE * 0.985;

positions.forEach(pos => {
  const posX = ORIGIN_X + pos.x * DX_SE + pos.y * DX_SW;
  const posY = ORIGIN_Y + pos.x * DY_SE + pos.y * DY_SW;
  blit(tileScaled, canvas, posX, posY);
});
...
console.log(`Saved clean, perfectly square island-16-grid.png (${trimW}x${trimH}) to ${outPath}`);
