...
const rawTile = PNG.sync.read(fs.readFileSync(path.join(tilesDir, 'tile-single.png')));

// 1. Precise color de-halo: replace any pale edge pixels (> 175 brightness with low saturation) with true grass/rock colors
for (let y = 0; y < rawTile.height; y++) {
  for (let x = 0; x < rawTile.width; x++) {
    const idx = (rawTile.width * y + x) << 2;
    const a = rawTile.data[idx + 3];
    if (a < 30) continue;

    const r = rawTile.data[idx];
    const g = rawTile.data[idx + 1];
    const b = rawTile.data[idx + 2];
    const brightness = (r + g + b) / 3;

    if (brightness > 175) {
      if (y < 160) {
        // In grass region: tint to rich lush green
        rawTile.data[idx] = Math.round(r * 0.70);
        rawTile.data[idx + 1] = Math.min(215, Math.round(g * 0.95));
        rawTile.data[idx + 2] = Math.round(b * 0.50);
      } else {
        // In cliff rock region: tint to rich earthen rock brown
        rawTile.data[idx] = Math.min(160, Math.round(r * 0.75));
        rawTile.data[idx + 1] = Math.min(110, Math.round(g * 0.65));
        rawTile.data[idx + 2] = Math.min(70, Math.round(b * 0.50));
      }
    }
  }
}
...
// 2. Measure geometry
let topPt = { x: 0, y: rawTile.height };
let leftPt = { x: rawTile.width, y: 0 };
let rightPt = { x: 0, y: 0 };

for (let y = 0; y < 140; y++) {
  for (let x = 0; x < rawTile.width; x++) {
    const a = rawTile.data[(rawTile.width * y + x) * 4 + 3];
    if (a > 180) {
...
console.log('Top:', topPt, 'Left:', leftPt, 'Right:', rightPt);
...
const SCALE = 0.52;
const fullTileScaled = resizePng(rawTile, SCALE);
...
const ORIGIN_X = 400 - fullTileScaled.width / 2;
const ORIGIN_Y = 120;

// Perfectly calibrated isometric step vectors (100% solid seamless overlap)
...
positions.forEach(pos => {
  ...
  blit(fullTileScaled, canvas, posX, posY);
});
...
console.log(`Saved pristine, solid seamless island-16-grid.png (${trimW}x${trimH}) to ${outPath}`);
